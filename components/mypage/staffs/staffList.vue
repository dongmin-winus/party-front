<template>
    <div>
        <div class="m-visual type01">
            <h3 class="title">{{ rep_district['name'] ? rep_district['name'] : $store.state.district.district }} 섬기는 사람들
            </h3>
        </div>
                    ...
                    <ul class="items custom-ul">
                        ...
                        <li v-for="(item, index) in registerInfo" :key="index" style="padding:10px 10px;">
                            <div class="item-container">
                                <div class="left">
                                    <div>
                                        {{ $store.state.district.district }}{{ transGroup(item.group) }}&nbsp;{{ item.after
                                    ...
                                    <div class="m-board-btns">
                                        <div class="m-btns type01">
                                            <div class="m-btn-wrap ">
                                                <button type="button" style="height:30px;" class="m-btn type01 bg-grey"
                                                    @click.prevent="debounceCancelRegister(item.id)">
                                                    취소
                                                </button>
                                            </div>
                                        </div>
                ...
                <div class="mt-12 m-tabs type01" v-if="computedCountySections.length > 1">
                    ...
                    <div class="m-tab-wrap">
                        <div class="m-tab active">
                            <span class="text">{{ rep_district['name'] ? rep_district['name'] :
                                this.$store.state.district.district }}&nbsp; {{ transGroup(group) }}</span>
                        </div>
                    </div>
                </div>
                <ul class="items custom-ul">
                            ...
                            <div class="item-container">
                                ...
                                <div class="right" v-if="item.catecode !== 'A0001'">
                                    <div class="m-board-btns">
                                        <div class="m-btns type01">
                                            <div class="m-btn-wrap">
                                                <button v-show="!item.id" type="button"
                                                    class="m-btn type01 height-full bg-revert-primary"
                                                    @click="setForm(item)">
                                                    등록
                                                </button>
                                                <button v-show="item.id" type="button"
                                                    class="m-btn type01 height-full bg-revert-orange"
                                                    @click="setForm(item)">
                                                    수정
                                                </button>
                                            </div>
                                            <div class="m-btn-wrap">
                                                <button type="button" class="m-btn type01 height-full bg-revert-red"
                                                    @click="openReminder(item, '삭제', '취소')">
        ...
        <modal v-if="activeModal" :noPaddingModal="true" :paddingModal="false" :cancelBtn="false">
            <template #outter>
                <div class="area-write area-staff">
                    <div class="wrap">
                        <div class="m-input-wrap">
                            <h3 class="m-input-title">직분</h3>
                            <Dropdown :menuTitle="'직분'" :activate="activeDropdown" :items="computedPositions"
                                :selected="positionSelected" @toggle="togglePosition" @change="changePosition"
                  ...
                  <div class="m-input-error" v-if="errors.position">{{errors.position[0]}}</div> -->
                        ...
                        <div class="m-board-btns" style="width:100%; display:flex; justify-content:space-around;">
                            <div class="m-btns type01">
                                <div class="m-btn-wrap">
                                    <button v-if="!form.id" type="button" class="m-btn type01 height-full bg-revert-primary"
                                        @click="store()">
                                        <!-- <button v-if="!form.id" type="button" class="m-btn type01 height-full bg-revert-primary" @click="storeDemo()"> -->
                                        등록
                                    </button>
                                    <button v-else type="button" class="m-btn type01 height-full bg-revert-orange"
                                        @click="updateItem()">
        ...
        <!-- 삭제 리마인더 -->
        <Reminder v-if="activeReminder" :title="'삭제 하시겠습니까?'" :excecute="excecuteName" :cancel="cancelName" :item="item"
            @excecute="remove" @cancel="closeReminder" />
    </div>
</template>
