    ...
    <div class="container">
      <div class="content">
        ...
        <div class="form-container">
          <SurveyItem v-for="(item, index) in buildArrWoHeader" :key="index" :item="item" :custom="true" :form_id="'32'" @valueChange="valueChange" />
...
export default {
  ...
  async mounted() {
    ...
    const response = await this.$axios.$get(`/api/forms/option/32`)
    this.form_id = '32'
  ...
  methods: {
    ...
    async submit() {
      //validation builderWohHeader 에서 required가 1인 항목의 id를 가져와서 values에 해당 id가 있는지 확인한다.
